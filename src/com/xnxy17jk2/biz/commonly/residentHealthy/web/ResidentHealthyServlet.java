...
/**
 * @项目名称: publicHealth
 * @包名: com.xnxy17jk2.biz.commonly.residentHealthy.web
 * @类名: ResidentHealthyServlet
 * @描述: 类说明
 * @作者: Guo_SY
 * @创建时间: 2020年5月7日 下午3:14:21
 */
public class ResidentHealthyServlet extends BaseServlet {
	...
	public void add(HttpServletRequest request, HttpServletResponse response) 
		...
		if (service.save(residentHealthy)) {
			response.getWriter().write(
					"<script>alert('添加成功'); window.location='residentHealthy?action=query';</script>");
			response.getWriter().flush();
		} else {
			response.getWriter().write(
					"<script>alert('添加失败'); window.location='residentHealthy?action=query';</script>");
			response.getWriter().flush();
		}
	}
	...
	public void update(HttpServletRequest request, HttpServletResponse response) 
		...
		if (service.update(residentHealthy)) {
			response.getWriter().write(
					"<script>alert('修改成功'); window.location='residentHealthy?action=query';</script>");
			response.getWriter().flush();
		} else {
			response.getWriter().write(
					"<script>alert('修改失败'); window.location='residentHealthy?action=query';</script>");
			response.getWriter().flush();
		}
	}
